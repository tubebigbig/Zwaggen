...
Compare two `.zwag.json` files side-by-side to see what changed and whether it's breaking. Built for PR review.
...
- **PR review.** Before approving a spec change, compare the PR branch's `.zwag.json` against `main`'s. Anything in the red bucket deserves a breaking-change note in the PR body.
